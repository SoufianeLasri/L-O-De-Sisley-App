...
class EtiquetteView: UIView {
    ...
    init( frame: CGRect, mainText: String, secondText: String ) {
        ...
        title.text = mainText
        title.font = UIFont( name: "Santana-Bold", size: 30.0 )
        title.textAlignment = .Center
        title.textColor = UIColor( red: 0.36, green: 0.37, blue: 0.54, alpha: 1.0 )
        self.addSubview( title )
        
        let subtitle = UILabel( frame: CGRect( x: 0, y: self.frame.height / 2 - 5, width: self.frame.width, height: 50 ) )
        subtitle.text = "+ " + secondText
        subtitle.font = UIFont( name: "Bellota-Italic", size: 21.0 )
        subtitle.textAlignment = .Center
        subtitle.textColor = UIColor( red: 0.89, green: 0.81, blue: 0.47, alpha: 1.0 )
        self.addSubview( subtitle )
        
        self.likeButton = LikeButton( frame: CGRect( x: self.frame.width - 40, y: 10, width: 30, height: 30 ) )
