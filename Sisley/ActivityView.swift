...
class ActivityView: UIView, CustomCarouselDelegate {
    ...
    init( frame: CGRect, data: [ String : Dictionary<String, NSObject> ] ) {
        ...
        self.carousel = CustomCarouselView( frame: CGRect( x: 0, y: etiquette.frame.maxY + 3, width: self.frame.width - 60, height: self.frame.height - etiquette.frame.maxY - 25 ), data: data[ "carousel" ]!, comments: false )
